...
const ElectronWalletManager = require('./ws_manager');
const config = require('./ws_config');
const async = require('async');
const AgGrid = require('ag-grid-community');
const wsmanager = new ElectronWalletManager();
...
const ElectronWalletAddressbook = require('./ws_addressbook');
...
let addressBook = new ElectronWalletAddressbook(ADDRESS_BOOK_DEFAULT_PATH);
...
let jtfr = {
    tFind: [
        "Electron Wallet",
        "https://github.com/BitNovex/bitnovex-wallet-electron",
        "BitNovex",
        "BTNX",
        "BitNovex-service"
    ],
...
function showInitialPage() {
    ...
    let versionInfo = document.getElementById('electronWalletVersion');
    if (versionInfo) versionInfo.innerHTML = WS_VERSION;
    let tsVersionInfo = document.getElementById('bitnovexServiceVersion'); // BitNovexServiceVersion
...
function handleAddressBook() {
    ...
    function renderItem(e) {
        ...
        if (myaddress && isSynced) {
            sendBtnx = `<button data-addressid="${data.key}" type="button" class="form-bt button-green ab-send" id="button-addressbook-panel-send">Send ${config.assetTicker}</button>`;
        }
    ...
    function loadAddressBook(params) {
        ...
        if (params) {
            ...
            if (params.name === 'default') {
                addressBook = new ElectronWalletAddressbook(ADDRESS_BOOK_DEFAULT_PATH);
            } else {
                addressBook = new ElectronWalletAddressbook(params.path, params.name, params.pass);
            }
        }
...
ipcRenderer.on('cleanup', () => {
    ...
    let htmlText = 'Terminating ElectronWallet...';
